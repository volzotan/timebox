...
void initButtons() {
  ...
  pinMode(PIN_CAMERA_FOCUS,    INPUT_PULLUP);
  pinMode(PIN_CAMERA_SHUTTER,  INPUT_PULLUP);
...
float voltageDivider(float input) {
  return ((input / 1024) * 3.3) / ( (float) VDRESISTOR2 / (VDRESISTOR1 + VDRESISTOR2) );
}
...
float getLiPoVoltageRaw(int cell) {
  switch (cell) {
    case 0: // whole battery
      return voltageDivider((float) analogRead(PIN_CELL_2));
      break;
